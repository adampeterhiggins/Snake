...
def turn(dir):
    global Direction
    if not np.array_equal(dir, (-1) * Direction):  # Don't allow snake to reverse direction
        Direction = dir
...
def SquareVertices(
    Posistion, size
):  # returns an array of corner posistions of square with center Posistion and half width size
